...
#include "generativeHMM.h"

void generateSequence(double pi[], double A[][N], double B[][M], int length) {
    int i, t;
    double prob;
    int currentState, nextState, observation;

    // Choose initial state based on pi
    prob = (double)rand() / RAND_MAX; 
    for(i = 0, currentState = -1; i < N && currentState == -1; i++) {
        if(prob < pi[i]) {
            currentState = i;
        }
        prob -= pi[i];
    ...
    for(t = 0; t < length; t++) {
        ...
        prob = (double)rand() / RAND_MAX;
        for(i = 0, observation = -1; i < M && observation == -1; i++) {
            if(prob < B[currentState][i]) {
                observation = i;
            }
            prob -= B[currentState][i];
        }
        printf("%c", 'a' + observation); // Assuming observations are characters

        // Transition to the next state
        prob = (double)rand() / RAND_MAX;
        for(i = 0, nextState = -1; i < N && nextState == -1; i++) {
            if(prob < A[currentState][i]) {
                nextState = i;
            }
            prob -= A[currentState][i];
        }
...
//    sprintf(fname, argv[1]);
    ...
    printf("\nGenerating sequence of length 100:\n");
    generateSequence(pi, A, B, 100);
