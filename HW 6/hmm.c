...
#include "hmm.h"
#define learningRate 10.0f
#define TEMPEARTURE 5.0f
...
//    sprintf(fname, argv[1]);
    ...
    while((iter < maxIters) && (newLogProb > logProb))
    {
        ...
        reestimateWV(step, A, B, W, V, logProb, T);
                 ...
                 int T)
{
    ...
    for(i = 0; i < N; ++i)
    {
        denom = 0.0;
        for (int k = 0; k < N; ++k)
            denom += exp(TEMPEARTURE * W[i][k]);
        for(j = 0; j < N; ++j)
        {
            numer = exp(TEMPEARTURE * W[i][j]);
                 ...
                 int T)
{
    ...
    for(i = 0; i < N; ++i)
    {
        denom = 0.0;
        for (int k = 0; k < N; ++k)
            denom += exp(TEMPEARTURE * V[i][k]);
        for(j = 0; j < N; ++j)
        {
            numer = exp(TEMPEARTURE * V[i][j]);
...
void reestimateWV(struct stepStruct *step, float A[N][N], float B[N][M], float W[N][N], float V[N][M], float logProb, int T) {
    for(int t = 0; t < T - 1; ++t) // t = 0,1,2,...,T-2
    {
        for(int i = 0; i < N; ++i)
        {
            for(int j = 0; j < N; ++j)
            {
                W[i][j] += (learningRate / logProb) * (step[t].diGamma[i][j] - step[t].gamma[i] * A[i][j]);
            }
            for(int j = 0; j < M; ++j)
            {
                V[i][j] += (learningRate / logProb) * (step[t].diGamma[i][j] - step[t].gamma[i] * B[i][j]);
            }
        }
    }
}
                  ...
                  int seed)
{
    ...
    for(i = 0; i < N; ++i)
    {
        if((random() & 0x1) == 0)
        {
            pi[i] = prob + (float)(random() & 0x7) / 8.0 * ftemp;
        }
        else
        {
            pi[i] = prob - (float)(random() & 0x7) / 8.0 * ftemp;
    ...
    for(i = 0; i < N; ++i)
    {
        ...
        for(j = 0; j < N; ++j)
        {
            if((random() & 0x1) == 0)
            {
                A[i][j] = prob + (float)(random() & 0x7) / 8.0 * ftemp;
            }
            else
            {
                A[i][j] = prob - (float)(random() & 0x7) / 8.0 * ftemp;
...
//    A[1][1] = .220;
    ...
    for(i = 0; i < N; ++i)
    {
        ...
        for(j = 0; j < M; ++j)
        {
            if((random() & 0x1) == 0)
            {
                B[i][j] = prob + (float)(random() & 0x7) / 8.0 * ftemp;
            }
            else
            {
                B[i][j] = prob - (float)(random() & 0x7) / 8.0 * ftemp;
